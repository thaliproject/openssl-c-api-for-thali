...
static void free_openssl_structures(EVP_PKEY *key, X509 *cert, STACK_OF(X509) *certstack);
...
PKCS12* create_PKCS12_stream(int keysize, const char *password)
{
	...
	if(privkey == NULL)
	{
		free_openssl_structures(privkey, x509_cert, cacertstack);
		cleanup_openssl();
		return NULL;
	}
...
void free_openssl_structures(EVP_PKEY *key, X509 *cert, STACK_OF(X509) *certstack)
{
	if(certstack != NULL)
	{
